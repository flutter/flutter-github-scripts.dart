...
void main(List<String> args) async {
  ...
  for (var item in issues) {
    ...
    if (issue.milestone != null &&
      ...
      if (issue.milestone.title == '[DEPRECATED] Goals' ||
          ...
          (issue.milestone.title == 'Overdue')) continue;
  ...
  List<String> noOwnedIssues = <String>[];
  noOwnedIssues.addAll(interesting);
  clustersInterestingOwned.clusters.keys
      .forEach((interestingUser) => noOwnedIssues.remove(interestingUser));

  print(
      '## Issues owned by core team members at Google (${interestingOwnedIssues.length})\n');
  print(
      'x̄ = ${clustersInterestingOwned.mean()}, σ = ${clustersInterestingOwned.stdev()}');
  ...
  print(
      '## Owned issues with no milestone by owner (${noMilestones.length})\n');
  print(clustersNoMlestones.toMarkdown(
      sortType: ClusterReportSort.byKey,
      skipEmpty: true,
      showStatistics: false));

  print(
      '## Issues with milestones and no owners (${noAssigneesYetMilestoned.length})');
